...
import { GameDto, GameType, PlayerDto, Question, TypedError } from "../types/types";
import { getAccessTokenFp } from "./AuthorizationClient";
import { getRequest, getTextRequest, postRequest } from "./request";
...
export const submitAnswer = (gameId: string, questionId: string, answerId: string) => {
  const makeRequest = (headers: ApiHeaders) => getRequest<void>(`api/games/${gameId}/questions/${questionId}/answers/${answerId}`, headers);
